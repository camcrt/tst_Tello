...
def callback1(data):
    ...
    def regul_x():
        rospy.loginfo(data.pose.position.x - x_drone2)

        delta = 0.3
        distance = 1

        if distance - delta < data.pose.position.x - x_drone2  < distance + delta :
            cmd.linear.x = 0
            pub.publish(cmd)
            return

        if data.pose.position.x - x_drone2 > distance + delta : 
            cmd.linear.x = 0.5
            pub.publish(cmd)
